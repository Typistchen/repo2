...
u8 Handle[U4_DATA_LEN];
u8 connect_flag[4] = {1,1,1,1} ;	// 1 -- 表示未连接
UltraStruct Ultrasonic = {0,0,0,0,0,0,0,0};
char err[4]={0,0,0,0} ;
/*
********************************************************************************************************************
...
void UART4_DMATransfer(uint32_t *BufferSRC, uint32_t BufferSize)//UASRT DMA发送设置
{
	...
	DMA1_Stream4->NDTR = BufferSize;    //配置BUFFER大小
	DMA1_Stream4->M0AR = (uint32_t)BufferSRC;   //配置地址
	DMA1_Stream4->CR |= DMA_SxCR_EN;	//打开DMA,开始发送
}
...
void UART4_IRQHandler(void)
{
	...
	{
		t= USART_ReceiveData(UART4);//读取数据 注意：这句必须要，否则不能够清除中断标志位。
		Uart4_Rec_Len = U4_DATA_LEN - DMA1_Stream2->NDTR;	//算出接本帧数据长度

		//***********帧数据处理函数************//
		
			//		UART4_DMATransfer((u32*)rec_data_u4,sizeof(rec_data_u4));
	...
	else if(USART_GetITStatus(UART4, USART_IT_TC) != RESET)
	{
		...
		DMA1_Stream4->CR &= (uint32_t)(~DMA_SxCR_EN);   //关闭DMA,发送完成
	}
}
/*
********************************************************************************************************************
...
{ 
	...
	DMA_Cmd(DMA1_Stream2,DISABLE); //关闭USART2 TX DMA1 所指示的通道

 	//DMA_SetCurrDataCounter(DMA1_Channel5,Uart1_DMA_Len);//DMA通道的DMA缓存的大小
	DMA1_Stream2->NDTR = U4_DATA_LEN;
 	//DMA_Cmd(DMA1_Channel5, ENABLE);                    
	DMA_Cmd(DMA1_Stream2,ENABLE); //使能USART2 TX DMA1 所指示的通道
	/* 清除标志位 */
   
}

void Upper_f_Computer(float data1,float data2,float data3,float data4)
{
	u8 i,sum;
	...
	Send_DATA[20]=sum;
	UART4_DMATransfer((u32*)Send_DATA,21);
}
